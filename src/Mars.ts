...
export class Mars {
  ...
  constructor (sceneInstance: SceneInstance) { // }, waterMaterial: WaterMaterial) {
    ...
    sceneInstance.scene.registerBeforeRender(() => {
        ...
        // this.debugFirstMeshCoordinate(this.mesh as BABYLON.Mesh);
  ...
  createTheShark (sceneInstance: SceneInstance) {
    ...
    new MarsMesh(sceneInstance.scene).getObservableMesh()
        .subscribe(sharkMesh => {
            this.mesh = sharkMesh;
            // this.mesh.getChildren().forEach(
            //     mesh => {
  ...
  animateShark(deltaTime: number): void {
    // console.log(this.swimming.getValue());
      if (this.mesh && this.swimming.getValue()) {
